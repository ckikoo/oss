...
type IObjectRepo interface {
	...
	MarkAllNotLatest(ctx context.Context, bucketName, objectKey string) error
	MarkVersionPurged(ctx context.Context, bucketName, objectKey, versionID string) (*do.ObjectDo, error)
	PromotePreviousVersion(ctx context.Context, bucketName, objectKey string) (*do.ObjectDo, error)
