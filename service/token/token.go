...
func (s *Service) CreateDownloadToken(ctx context.Context, ak string, secure string, req *dto.CreateDownloadTokenReq) (*dto.CreateTokenResp, common.Errno) {
	token := genToken(ak, req.BucketName, req.ObjectKey, consts.DownloadMethod, consts.DownloadAction, req.ExpiresIn, secure)

	expireTime := time.Unix(req.ExpiresIn, 0)
	s.rds.CreateDownloadToken(ctx, token, gconv.String(req), time.Until(expireTime)*time.Second)

	return &dto.CreateTokenResp{
		Token:    token,
		ExpireAt: req.ExpiresIn,
	}, common.OK
}
